...
- **[references/qualification-framework.md](references/qualification-framework.md)** — IF/ONLY IF conditional patterns, transfer authorization blocks, info collection structure, variable strategy. Read when designing Steps 2-4 of the prompt.
- **[references/appointment-setting.md](references/appointment-setting.md)** — Complete appointment scheduling flow. Read when the use case includes booking, and always in Trojan mode (the Trojan script hands off to it).
- **[references/knowledge-base-split.md](references/knowledge-base-split.md)** — When (and when not) to move reference data out of the prompt into `kb-*.txt` files. Read before saving if the client provided supporting docs or if the drafted prompt is trending above ~10k tokens.
- **[references/trojan-horse.md](references/trojan-horse.md)** — Sales overlay script: segue trigger, missed-calls math (industry-adapted), frame-flip close, deploy-tonight urgency, appointment push, guardrails. Read in Trojan mode only.
- **[assets/voice-agent-template.md](assets/voice-agent-template.md)** — Master template for the regular receptionist prompt. Always use this as the base.
- **[assets/voice-agent-template-trojan-overlay.md](assets/voice-agent-template-trojan-overlay.md)** — The Trojan overlay block that layers on top of the regular template. Read in Trojan mode only.
...
Read [references/trojan-horse.md](references/trojan-horse.md) and [assets/voice-agent-template-trojan-overlay.md](assets/voice-agent-template-trojan-overlay.md).
...
2. **New Step in the Steps section** — inserted after the caller has asked 1-2 business questions or after ~60-90 seconds of rapport. This is the **Trojan segue**. Full script lives in references/trojan-horse.md. Keep the segue natural ("quick question while I have you") — never a hard break.
...
7. **Objection Handling** — add Trojan-specific objections: "how much does it cost", "I need to think about it", "send me something in writing", "we already have a receptionist". Scripts in references/trojan-horse.md.
8. **Guardrails in Notes** — from references/trojan-horse.md: never say "I'm a demo" unless directly asked; compress the math to one line if the caller is rushed; never break frame.
...
- [ ] All guardrails from references/trojan-horse.md are in Notes

---

## Step 3.5: Decide on knowledge base splits

Same decision for both prompts. If the client provided supporting docs (FAQs, price sheets, catalogs, policies, location lists, insurance carrier lists, long background material) **or** the drafted prompt is trending above ~10,000 tokens (estimate with `chars / 4`), read [references/knowledge-base-split.md](references/knowledge-base-split.md) and decide what to split into `kb-*.txt` files.

Non-negotiables — these always stay in the main prompt regardless of size: role, personality, steps, rules, objection handling, appointment-setting logic, **and (in Trojan mode) the entire Trojan overlay**. Only pure reference data is a candidate for splitting.

For each KB file you create:
- Save as plain `.txt` (not `.md`) alongside the prompt file, named `kb-<topic>.txt`.
- Remove the split content from the main prompt entirely — no duplicate summary.
- Add a short `## Knowledge Base` section near the bottom of the prompt (above "Notes") listing each KB file and *when* to consult it.
...
- **Trojan mode on a business where the math framework doesn't fit** (e.g. a SaaS with zero phone calls) → still try; Trojan references/trojan-horse.md has adapted variants, and for truly unfit cases the frame-flip + deploy-tonight urgency still lands even without the missed-calls math.
